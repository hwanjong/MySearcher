...
public class CommunityModule extends Parser {
	public CommunityModule(Document html) {
		super(html);
	}

	public ArrayList<SubContents> getNatePannContents() { // 링크, 제목, 내용, 날짜, 태그,
															// 이미지
	...
	public ArrayList<SubContents> getNaverKinContents() { // 링크, 제목, 날짜, 내용, 태그,
															// 이미지
		Document doc = this.html;
		ArrayList<SubContents> list = new ArrayList<SubContents>();
		...
		for (Element ori : original) {
			...
			if (temp != null) {
				...
				// 태그저장// System.out.println(ori.select("dd.txt_block").text());
