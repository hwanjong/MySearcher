...
public class ShoppingParser extends RequestParser {
	...
	public ArrayList<SubContents> getContents(UserRequest c, String param) {
		...
		switch (c) {
		case ElevenST:
			contents = getElevenStContents(url);
			break;
		case Aution:
			contents = getAutionContents(url);
			break;
		case Coupang:
			contents = getCoupangContents(url);
			break;
		case Timon:
			contents = getTimonContents(url);
			break;
		default:
			break;
		}
		...
		try {
			...
			for (Element e : original) {
				...
				if (img != null) {
					...
					title = e.select("div.item_title").first().select("a")
							.first();
